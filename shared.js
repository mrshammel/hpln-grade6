/* ============================================================
   GRADE 6 -- SHARED MODULE â" SHARED JAVASCRIPT MODULE
   ============================================================
...
function toggleTeacherUnlock() {
  if (isTeacherUnlocked()) {
    if (confirm('Deactivate teacher unlock mode?')) {
      ...
      alert('ð"' Teacher unlock deactivated.');
    }
    return;
  }
  const code = prompt('ð" Enter teacher access code:');
  if (!code) return;
  if (code.trim() === 'shammel2026') {
    localStorage.setItem('g6-teacher-unlock', 'true');
    applyTeacherUnlock();
    alert('ð"" Teacher unlock activated! All lessons and quizzes are now accessible.');
  } else {
    alert('â Invalid code.');
  }
}
...
function applyTeacherUnlock() {
  ...
  document.querySelectorAll('[id$="-gate-msg"]').forEach(msg => {
    if (!msg.textContent.includes('All activities complete')) {
      msg.innerHTML = 'ð"" Teacher unlock active â" quiz available.';
...
function showUnlockBadge() {
  ...
  badge.title = 'Teacher Unlock Active â" Click to deactivate';
  badge.textContent = 'ð""';
...
function toggleTheme() {
  ...
  if (btn) btn.innerHTML = light ? 'ð Dark' : 'âï¸ Light';
...
function loadTheme() {
  ...
  const btn = document.getElementById('themeBtn');
  if (btn) btn.innerHTML = document.body.classList.contains('light') ? 'ð Dark' : 'âï¸ Light';
}
...
function checkSignIn() {
  ...
  if (name && email) {
    ...
    if (display) {
      display.innerHTML = 'ð'¤ ' + name + ' <a href="#" onclick="event.preventDefault();studentSignOut()" title="Sign out" style="color:var(--text3);text-decoration:none;margin-left:6px;font-size:.75rem;opacity:.7">â-</a>';
    }
  } else {
...
function renderTeacherDashboard() {
  ...
  grades.forEach(g => { html += `<tr><td>${g.name}</td><td>${g.email}</td><td>${g.quizId}</td><td>${g.score}/${g.total}</td><td>${g.percentage}%</td><td>${g.passed ? 'â...' : 'â'}</td><td>${g.attempts}</td><td>${g.timestamp ? g.timestamp.slice(0, 10) : ''}</td></tr>`; });
  html += '</tbody></table>';
  el.innerHTML = html;
...
function pickAns(qid, oi) {
  ...
  if (oi === q.ans) {
    ...
    fb.textContent = 'â... Correct! ' + q.explain;
    s.score++; s.answered++;
    ...
    if (q.strikes === 1 && !q.isReplacement) {
      ...
      if (rep) {
        ...
        fb.innerHTML = 'â ' + q.explain + '<br><br>â¡ <strong>Let\'s try a similar questionâ¦</strong>';
      ...
      if (bk) {
        fb.className = 'feedback-box show wrong';
        fb.innerHTML = 'â ' + q.explain + '<br><br>ð"- <strong>Time for a mini-lesson! Watch a video and answer 3 questions.</strong>';
        setTimeout(() => { s.inRemediation = true; s.remOutcome = oc; s.remCorrect = 0; pickRemQ(qid); renderAQ(qid); }, 2800);
        return;
      }
    }
    fb.className = 'feedback-box show wrong';
    fb.textContent = 'â ' + q.explain;
    nb.classList.add('show');
  }
}
...
function renderRem(qid) {
  ...
  if (s.remIdx === 0) {
    area.innerHTML = `<div class="remediation-view active"><div class="remediation-header"><h2>ð"- Mini-Lesson: ${s.remOutcome}</h2><p>Watch this video, then answer 3 questions correctly to continue.</p></div><div class="glass-card"><div class="video-wrap"><iframe src="${videoSrc}" title="Remediation" allow="accelerometer;autoplay;clipboard-write;encrypted-media;gyroscope;picture-in-picture" allowfullscreen></iframe></div></div><div style="text-align:center;margin-top:20px"><button class="nav-btn primary" onclick="startRemQ('${qid}')">I've Watched â" Start Questions â'</button></div></div>`;
    return;
  }
  renderRemQ(qid);
}
...
function renderRemQ(qid) {
  ...
  area.innerHTML = `<div class="remediation-view active"><div class="remediation-header"><h2>ð"- Mini-Lesson: ${s.remOutcome}</h2><p>Get all 3 correct to return! (${s.remCorrect}/3)</p></div><div class="question-card"><div class="q-outcome">${s.remOutcome} â" Remediation</div><div class="q-text">${q.q}</div><div class="options">${q.opts.map((o, j) => `<button class="option-btn" onclick="pickRemAns('${qid}',${j})"><span class="option-letter">${letters[j]}</span>${o}</button>`).join('')}</div><div class="feedback-box" id="${qid}-rfb"></div><div class="quiz-nav"><button class="quiz-next-btn" id="${qid}-rnxt" onclick="nextRemQ('${qid}')">Next â'</button></div></div></div>`;
}
...
function pickRemAns(qid, oi) {
  ...
  if (oi === q.ans) {
    ...
    s.remCorrect++;
    nb.classList.add('show');
    if (s.remCorrect >= 3) nb.textContent = 'Return to Quiz â"';
  } else {
    btns[oi].classList.add('wrong-answer');
    btns[q.ans].classList.add('correct-answer');
    fb.className = 'feedback-box show wrong';
    fb.innerHTML = 'â ' + q.explain + '<br><br>ð" <strong>Let\'s try again from the startâ¦</strong>';
...
function showResults(qid) {
  ...
  const pass = pct >= 80;
  const area = document.getElementById(qid + '-area');
  // Extract lesson number from qid (e.g. "quiz1" â' 1)
  ...
  let bh = '';
  if (pass) {
    bh = `<button class="results-btn primary" onclick="goTo('home')">Continue â'</button><button class="results-btn" onclick="initQuiz('${qid}', quizState['${qid}'].banks)">ð" Retake</button>`;
  } else {
    bh = `<button class="results-btn primary" onclick="goTo('lesson${ln}')">ð"- Review Lesson</button><button class="results-btn" onclick="initQuiz('${qid}', quizState['${qid}'].banks)">ð" Retry</button>`;
  }
  area.innerHTML = `<div class="quiz-results-view active"><div class="score-circle ${pass ? 'pass' : 'fail'}">${pct}%</div><p class="quiz-msg">${pass ? 'ð Excellent! You\'ve mastered this lesson!' : 'ð"- You need 80% to unlock the next lesson.'}</p><p class="quiz-msg-sub">${s.score}/${s.total} (${pct}%)${pass ? '' : ' â" Need ' + Math.ceil(s.total * .8)}</p><div style="display:flex;justify-content:center;gap:12px;flex-wrap:wrap">${bh}</div></div>`;
}
...
function checkGate(lesson) {
  ...
  if (btn && (allDone || isTeacherUnlocked())) {
    ...
    btn.style.opacity = '1';
    btn.style.cursor = 'pointer';
    if (msg && allDone) { msg.innerHTML = 'â... All activities complete! You may now take the quiz.'; msg.style.color = 'var(--accent)'; }
    else if (msg && isTeacherUnlocked()) { msg.innerHTML = 'ð"" Teacher unlock active â" quiz available.'; msg.style.color = 'var(--accent)'; }
...
function revealAnswer(btn) {
  ...
  btn.textContent = isOpen ? 'ð Hide Answers' : 'ð' Reveal Answers';
}
...
function initReadToMe() {
  ...
  toolbar.innerHTML = `
    <button class="tts-toggle" id="ttsMainBtn" onclick="toggleTTS()" title="Read to Me">ð"</button>
    <span class="tts-toggle-label">Read to Me</span>
    <div class="tts-controls" id="ttsControls" style="display:none">
      <button class="tts-btn" onclick="ttsBack()" title="Previous section">â®</button>
      <button class="tts-btn" id="ttsPauseBtn" onclick="ttsPause()" title="Pause">â¸</button>
      <button class="tts-btn" onclick="ttsForward()" title="Next section">â-</button>
      <button class="tts-btn" onclick="ttsStop()" title="Stop" style="color:var(--danger)">â¹</button>
...
function toggleTTS() {
  ...
  document.getElementById('ttsMainBtn').innerHTML = 'ð"';
  document.querySelector('.tts-toggle-label').textContent = 'Readingâ¦';
  readCurrentSection();
}
...
function ttsPause() {
  ...
  if (ttsState.paused) {
    ...
    ttsState.paused = false;
    document.getElementById('ttsPauseBtn').innerHTML = 'â¸';
...
function initDrawingCanvas(containerId, opts = {}) {
  ...
  container.innerHTML = `
    <div class="tool-card drawing-canvas-wrap">
      <div class="tool-header"><span class="tool-icon">ð¨</span> ${title}</div>
      ...
      <div class="drawing-toolbar">
        ...
        <div class="drawing-tools">
          ...
          <button class="tool-btn active" data-tool="pen" title="Pen">âï¸ Pen</button>
          <button class="tool-btn" data-tool="eraser" title="Eraser">ð§¹ Eraser</button>
          <button class="tool-btn" data-tool="undo" title="Undo">â©ï¸</button>
          <button class="tool-btn" data-tool="redo" title="Redo">âªï¸</button>
          <button class="tool-btn" data-tool="clear" title="Clear">ð--'ï¸ Clear</button>
        </div>
      </div>
      <canvas id="${containerId}-canvas" width="${width}" height="${height}" class="drawing-canvas"></canvas>
      <div class="tool-actions">
        <button class="tool-action-btn secondary" onclick="downloadDrawing('${containerId}')">ð'¾ Save as Image</button>
        <button class="tool-action-btn primary" onclick="submitDrawing('${containerId}')">ð"¤ Submit Drawing</button>
...
function downloadDrawing(containerId) {
  ...
  if (status) { status.textContent = 'â... Image saved!'; setTimeout(() => status.textContent = '', 3000); }
}
...
function submitDrawing(containerId) {
  ...
  const status = document.getElementById(containerId + '-status');
  if (status) { status.textContent = 'â... Drawing saved! (Submit to teacher coming soon)'; setTimeout(() => status.textContent = '', 4000); }
...
function initPhotoCapture(containerId, opts = {}) {
  ...
  container.innerHTML = `
    <div class="tool-card photo-capture-wrap">
      ...
      <div class="photo-options">
        <button class="tool-action-btn primary" id="${containerId}-cameraBtn">ð"· Take Photo</button>
        <span class="tool-or">or</span>
        <label class="tool-action-btn secondary" for="${containerId}-fileInput">ð" Upload Photo</label>
      ...
      <div class="photo-camera-area" id="${containerId}-cameraArea" style="display:none">
        ...
        <div class="photo-camera-controls">
          <button class="tool-action-btn primary" id="${containerId}-captureBtn">ð"¸ Capture</button>
          <button class="tool-action-btn secondary" id="${containerId}-switchBtn">ð" Switch Camera</button>
          <button class="tool-action-btn secondary" id="${containerId}-closeCam">â- Close</button>
        </div>
      </div>
      ...
      <div class="tool-actions">
        <button class="tool-action-btn primary" onclick="submitPhotos('${containerId}')">ð"¤ Submit Photo(s)</button>
        <span class="tool-status" id="${containerId}-status"></span>
      </div>
    </div>`;
  ...
  function renderGallery() {
    ...
    if (!photos.length) { gal.innerHTML = '<p class="photo-empty">No photos yet â" take or upload up to ' + maxPhotos + '</p>'; return; }
    gal.innerHTML = photos.map((p, i) => `
      <div class="photo-thumb-wrap">
        <img src="${p}" class="photo-thumb" alt="Photo ${i+1}">
        <button class="photo-remove" onclick="removePhoto('${containerId}',${i})" title="Remove">â-</button>
      </div>
    `).join('');
  }
...
function submitPhotos(containerId) {
  ...
  const status = document.getElementById(containerId + '-status');
  if (status) { status.textContent = 'â... Photos saved! (Submit to teacher coming soon)'; setTimeout(() => status.textContent = '', 4000); }
